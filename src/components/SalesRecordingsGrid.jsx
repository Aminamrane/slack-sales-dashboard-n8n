...
import { Search, Video, FileText, ChartNoAxesCombined, Users, ArrowUpRight, RefreshCw, FolderOpen, Share2, LoaderCircle, Phone } from 'lucide-react';
...
// `calls` : appels Allo du mois par e-mail ({ byEmail, total, loading }) — demande dev 2026-09-24.
export default function SalesRecordingsGrid({data,loading,videosLoading,error,onRefresh,refreshing,onSelectSales,avatars={},darkMode,calls}) {
  ...
  const callsOf=s=>calls?.byEmail?.[s.email.toLowerCase()];
  const totals=[{Icon:Users,value:sales.length,label:'Sales actifs'},{Icon:Phone,value:calls?.loading?null:calls?.total,label:'Appels ce mois-ci'},{Icon:ChartNoAxesCombined,value:sales.reduce((n,s)=>n+(s.nb_scored||0),0),label:'Analyses'},{Icon:Video,value:data?.totals?.videos,label:'Vidéos'},{Icon:FileText,value:data?.totals?.transcriptions,label:'Notes & transcriptions'}];
