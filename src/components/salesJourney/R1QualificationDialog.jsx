...
export default function R1QualificationDialog({lead,onClose,onSave,dark=false}){
 ...
 {needsDate&&<label className="sj-date">{result==='rescheduled'?'Nouvelle date du R1':'Date et heure du R2'}<input aria-label={result==='rescheduled'?'Nouvelle date du R1':'Date et heure du R2'} type="datetime-local" value={date} disabled={busy} onChange={e=>{setDate(e.target.value);setError('');}}/></label>}
