...
export default function QualificationDialog({lead,stage='r2',onClose,onSave,dark=false}) {
  ...
  const [attended,setAttended]=useState(!['no_show','reporte','annule'].includes(old));
  ...
  const options=attended ? RESULTS : [['no_show','Client absent','Le rendez-vous n’a pas eu lieu',UserRoundX],['reporte','À reprogrammer','Choisir une nouvelle date',CalendarClock],['annule','Rendez-vous annulé','Sans nouvelle date prévue',CircleX]];
  async function save(continueContract) {
    if(busy)return;
    if(result==='reporte' && !date){setError('Choisissez la nouvelle date du rendez-vous.');return;}
  ...
  return createPortal(<div className={`sj-overlay ${dark?'sj-dark':''}`} onClick={e=>{if(e.target===e.currentTarget&&!busy)onClose();}}>
    <section ref={ref} tabIndex={-1} className="sj-dialog" role="dialog" aria-modal="true" aria-labelledby="sj-title">
      ...
      {result==='reporte'&&<label className="sj-date">Nouveau rendez-vous<input aria-label="Nouveau rendez-vous" type="datetime-local" value={date} disabled={busy} onChange={e=>setDate(e.target.value)}/></label>}
