 ...
 *
 * @note Aucun appel de modèle depuis cette page : le ticket entre dans la file
 * `bot_ia_prompts` et c'est l'agent interne qui le traite puis le passe en
 * `traite`. La page suit ce statut via l'API backend (GET /bot-ia-prompts/{id}).
 *
 ...
 *
 * @warning Accès NOMINATIF côté backend (ALLOWED_EMAILS de bot_ia_prompts.py),
 * qui fait foi. Côté front, l'onglet suit les rôles de ces personnes
 * (SUB_TICKETS_ROLES) : aucune adresse dans ce dépôt public.
 */
...
export default function CeoBotIaView() {
  ...
  // Les rôles ouvrent la page ; la liste nominative du backend décide ensuite.
  useEffect(() => {
    const u = apiClient.getUser();
    if (!u || !SUB_TICKETS_ROLES.has(u.role)) {
      navigate("/");
      return;
    }
    setUserRole(u.role);
  ...
  /** @brief Statut de chaque ticket d'archives, renvoyé par la liste du backend. */
  const fetchArchiveStatuts = (rows) => {
    if (!rows?.length) return;
    setArchiveStatutById((prev) => {
      const next = { ...prev };
      for (const row of rows) if (row?.id && row.statut) next[row.id] = row.statut;
      return next;
    });
  };
  ...
  /**
   * @brief Sonde `GET /api/v1/bot-ia-prompts/{id}` jusqu'à ce que le ticket `promptId` passe en `traite`.
   ...
   * rechargement.
   */
  const beginWait = (botId, promptId, page) => {
    const cancelled = { v: false };
    abortRef.current = { abort: () => { cancelled.v = true; clearTimeout(pollTimerRef.current); } };
    const poll = async () => {
      if (cancelled.v) return;
      try {
        const data = await apiClient.get(`/api/v1/bot-ia-prompts/${encodeURIComponent(promptId)}`);
  ...
  useEffect(() => {
    ...
    beginWait(p.botId, p.promptId, resumedParsed.mention);
  }, [authChecked]);
  ...
  const handleSend = async () => {
    ...
    //    son auteur étant recopié du jeton côté serveur ; l'API renvoie son id.
    let promptId = null;
    try {
      const created = await apiClient.post("/api/v1/bot-ia-prompts", { content: agentContent, source: "bot_ia" });
    ...
    beginWait(botId, promptId, parsed.mention);
  };
