      ...
      <div style={{ animation: 'pcReveal 0.5s cubic-bezier(0.4,0,0.2,1) both', fontFamily: "'Inter', 'SF Pro Display', -apple-system, BlinkMacSystemFont, 'Helvetica Neue', sans-serif", WebkitFontSmoothing: 'antialiased', MozOsxFontSmoothing: 'grayscale', letterSpacing: '-0.011em' }}>
        <div style={{ display: 'flex', alignItems: 'stretch', minHeight: '100vh' }}>
          ...
          <div style={{
            ...
            {/* Logo header */}
            <div style={{ padding: '18px 16px 14px', borderBottom: `1px solid ${C.border}`, marginBottom: 8, paddingTop: 80 }}>
              <div style={{
                ...
                return (
                  ...
                  >
                    <div style={{
                      ...
                      backgroundColor: isActive ? (darkMode ? '#111827' : '#fff') : (darkMode ? 'rgba(255,255,255,0.40)' : 'rgba(0,0,0,0.30)'),
                      WebkitMaskImage: `url(${item.iconSrc})`, WebkitMaskSize: 'contain', WebkitMaskRepeat: 'no-repeat', WebkitMaskPosition: 'center',
                      maskImage: `url(${item.iconSrc})`, maskSize: 'contain', maskRepeat: 'no-repeat', maskPosition: 'center',
                    }} />
                    <span style={{ fontSize: 13, fontWeight: isActive ? 600 : 500 }}>{item.label}</span>
                    {item.key === 'perf_closing' && clients.length > 0 && (
                      <span style={{
                        marginLeft: 'auto', fontSize: 11, fontWeight: 600, minWidth: 20, textAlign: 'center',
                        padding: '1px 6px', borderRadius: 10,
                        background: isActive ? (darkMode ? '#111827' + '30' : 'rgba(255,255,255,0.2)') : C.accent + '15',
          ...
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', padding: '8px 8px 8px 0', gap: 12 }}>
            ...
            <div style={{ flex: 1, marginLeft: 8, display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              {/* ═══ DASHBOARD ═══════════════════════════════════════════ */}
              {TABS[activeTab].key === 'dashboard' && (
                <div className="pc-scroll" style={{ flex: 1, overflow: 'auto', maxHeight: 'calc(100vh - 120px)' }}>
                  {dashboard ? (<>
                    {/* KPI Hero Cards */}
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
                      {[
                        ...
                        { label: 'Résiliés', value: (dashboard.counters?.resilie ?? 0) + (dashboard.counters?.self_resiliation ?? 0), color: '#ef4444', sub: 'Résiliation + Self' },
                        { label: 'Autres', value: (dashboard.counters?.pause ?? 0) + (dashboard.counters?.sans_suite ?? 0) + (dashboard.counters?.liquidation ?? 0) + (dashboard.counters?.retractation ?? 0), color: '#f59e0b', sub: 'Pause, sans suite, etc.' },
                      ].map((kpi, i) => (
                        <div key={i} style={{
