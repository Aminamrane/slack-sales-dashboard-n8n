...
/**
 * Sert les fonctions serveur additives (Sub Tickets…) pendant `npm run dev`.
 *
...
function localApiDev() {
  const ROUTES = {
    '/api/bot-ia/chat': 'api/bot-ia/chat.mjs',
    '/api/bot-ia/statut': 'api/bot-ia/statut.mjs',
  }
  const cache = new Map()
...
export default defineConfig(({ mode }) => {
  ...
  // charge que les VITE_* et ANTHROPIC_API_KEY resterait introuvable.
  // Rien ne fuit pour autant dans le bundle : Vite n'inline que import.meta.env.VITE_*.
  ...
  return {
    ...
    // .data/ est un registre d'exécution (écrit à chaud) : on ne veut pas
    // qu'une écriture y déclenche un rechargement de page en dev.
